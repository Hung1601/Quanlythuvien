...
void Register(Staff& s, string &username, string &password) {
    int i = 0;
    for(i=0;i<size(sf);i++){  // Kiểm tra tài khoản đã tồn tại hay chưa
...
bool Login(const string username, string password){
    for(auto& staff : sf ){  // Kiểm tra tài khoản, mật khẩu 
...
void changePassword(Staff* &s, string username, string currentPassword, string newPassword) { // Đổi mật khẩu
    if (s->Getpassword() == currentPassword) { // Kiểm tra tài khoản, mật khẩu 
        for(auto& staff : hs ){  // Cập nhật trong vector
...
int main(){
    ...
    bool isSearch = false; // Biến xác định đang trong quá trình chỉnh sửa
        ...
        switch (choice) {
            ...
            case 2:
                ...
                if(Login(username,password)){
                    ...
                    for (auto& staff : sf) { // Xác định đối tượng đang sử dụng
                        if (staff.Getaccount() == username && staff.Getpassword() == password) {
                        myStaffPtr = &staff;
                        break;
                        }
